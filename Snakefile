...
FTP = FTPRemoteProvider(username="tesk-1", password=[REDIGED])
...
single_end_samples = ["ntc.ics.muni.cz/upload/snakemake/bcl2fastq/" + row["library"] + "/raw_fastq/" + row["sample_name"] + ".fastq.gz" for  index, row in sample_tab.iterrows() if config["libraries"][row["library"]]["lib_reverse_read_length"] == 0]
first_pair_end_samples = ["ntc.ics.muni.cz/upload/snakemake/bcl2fastq/" + row["library"] + "/raw_fastq/" + row["sample_name"] + "_R1.fastq.gz" for  index, row in sample_tab.iterrows() if config["libraries"][row["library"]]["lib_reverse_read_length"] != 0]
second_pair_end_samples = ["ntc.ics.muni.cz/upload/snakemake/bcl2fastq/" + row["library"] + "/raw_fastq/" + row["sample_name"] + "_R2.fastq.gz" for  index, row in sample_tab.iterrows() if config["libraries"][row["library"]]["lib_reverse_read_length"] != 0]
...
rule all:
    input: FTP.remote(all_sample_inputs)  
